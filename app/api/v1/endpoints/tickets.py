...
def listar_tickets_usuario(user_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def actualizar_ticket(ticket_id: int, ticket_update: TicketUpdate, db: Session = Depends(get_db)):
