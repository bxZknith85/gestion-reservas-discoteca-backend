...
class TestCreateTablePrice:
    def test_crear(self, client):
        response = client.post(BASE + "/", json=TABLE_PRICE_DATA)
    ...
    def test_crear_datos_invalidos(self, client):
        response = client.post(BASE + "/", json={"table_id": -1})
...
class TestListTablePrices:
    ...
    def test_listar_por_mesa(self, client):
        response = client.get(f"{BASE}/table/3")
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        assert isinstance(data, list)
...
class TestUpdateTablePrice:
    def test_actualizar(self, client):
        response = client.put(f"{BASE}/1", json=TABLE_PRICE_UPDATE)
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        assert data["price"] == TABLE_PRICE_UPDATE["price"]

    def test_actualizar_no_existente(self, client):
        response = client.put(f"{BASE}/999", json=TABLE_PRICE_UPDATE)
        assert response.status_code == status.HTTP_404_NOT_FOUND
        assert "Precio de mesa no encontrado" in response.json()["detail"]
